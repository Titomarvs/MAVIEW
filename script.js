...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Attach event listeners to all forward and backward buttons
  document.querySelectorAll(".btf").forEach((button, index) => {
    button.addEventListener("click", () => scrollContainer(scrollContainers[index], -1));
  });

  document.querySelectorAll(".btw").forEach((button, index) => {
    button.addEventListener("click", () => scrollContainer(scrollContainers[index], 1));
  });
});
...
async function showSearchResults() {
  ...
  try {
    ...
    if (data.results.length === 0) {
      dropdown.innerHTML = "<p style='color: white; padding: 10px;'>No results found.</p>";
    } else {
      data.results.slice(0, 5).forEach((movie) => {
        const movieItem = document.createElement("a");
        movieItem.href = `movie_details/movie_details.html?id=${movie.id}`;
        ...
        movieImage.classList.add("dropdown-image");

        const movieTitle = document.createElement("span");
        movieTitle.textContent = movie.title;
        movieTitle.classList.add("dropdown-title");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Fetch results if on search.html
...
async function fetchSearchResults(query) {
  ...
  try {
      ...
      resultsContainer.innerHTML = `<p style="color: white; text-align: center;">Results for: <strong>${query}</strong></p>`;
          ...
          resultsContainer.innerHTML = data.results.map(movie => `
              <div style="text-align: center; padding: 10px; display: inline-block;">
                  <img 
                      ...
                      alt="${movie.title}" 
                      style="border-radius: 1.2rem; cursor: pointer;" 
                      onclick="window.location.href='movie_details/movie_details.html?id=${movie.id}'"
                  />
